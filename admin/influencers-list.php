...
$orderby_array = ['id','firstname','email','phone','orders','date_added','coupons','balance'];
...
$influencers = $WI_Influencer->db->get_results(
    "SELECT u.*, COALESCE(c.orders,0) as orders, COALESCE(i.coupons,0) as coupons, COALESCE(b.balance,0) as balance FROM $users_tbl as u
    LEFT JOIN (SELECT count(id) as orders, user_id FROM $commissions_tbl WHERE 1 GROUP BY user_id) as c ON u.id = c.user_id
    LEFT JOIN (SELECT count(id) as coupons, user_id FROM $influencer_coupons_tbl WHERE 1 GROUP BY user_id) as i ON u.id = i.user_id
    LEFT JOIN ($balance_sql) as b ON b.user_id = u.id
    WHERE u.user_role LIKE 'influencer' ORDER BY $order_by $order"
);

?>
<div class="wrap">
    <h1 class="wp-heading-inline"><?=wi_influencer_display_name(false)?></h1>
    <a href="<?=admin_url('admin.php?page=wi-add-influencer')?>" class="page-title-action">Add New</a>
    ...
    <form id="posts-filter" method="get">
        ...
        <input type="hidden" name="page" class="post_type_page" value="woocommerce-influencers">
            ...
            <thead>
                <tr> 
                    ...
                    <th scope="col" id="title" class="manage-column column-id column-primary sortable <?=wi_get_inverse_order_type($order_by,'id',$order)?>">
                        <a href="<?=admin_url('admin.php?page=woocommerce-influencers'.wi_get_order_by_get_vars($order_by,'id',$order))?>">
                            <span>ID</span>
                    ...
                    <th scope="col" id="title" class="manage-column column-title column-primary sortable <?=wi_get_inverse_order_type($order_by,'firstname',$order)?>">
                        <a href="<?=admin_url('admin.php?page=woocommerce-influencers'.wi_get_order_by_get_vars($order_by,'firstname',$order))?>">
                            <span>Name</span>
                            <span class="sorting-indicator"></span>
                        </a>
                    </th>
                    ...
                    <th scope="col" id="email" class="manage-column column-email sortable <?=wi_get_inverse_order_type($order_by,'email',$order)?>">
                        <a href="<?=admin_url('admin.php?page=woocommerce-influencers'.wi_get_order_by_get_vars($order_by,'email',$order))?>">
                    ...
                    <th scope="col" id="orders" class="manage-column column-orders sortable  <?=wi_get_inverse_order_type($order_by,'coupons',$order)?>">
                        <a href="<?=admin_url('admin.php?page=woocommerce-influencers'.wi_get_order_by_get_vars($order_by,'coupons',$order))?>">
                            <span>Coupons</span>
                            <span class="sorting-indicator"></span>
                        </a>
                    </th>
                    <th scope="col" id="orders" class="manage-column column-orders sortable  <?=wi_get_inverse_order_type($order_by,'orders',$order)?>">
                        <a href="<?=admin_url('admin.php?page=woocommerce-influencers'.wi_get_order_by_get_vars($order_by,'orders',$order))?>">
                            <span>Orders</span>
                            <span class="sorting-indicator"></span>
                        </a>
                    </th>
                    ...
                    <th scope="col" id="date" class="manage-column column-date sortable  <?=wi_get_inverse_order_type($order_by,'date_added',$order)?>">
                        <a href="<?=admin_url('admin.php?page=woocommerce-influencers'.wi_get_order_by_get_vars($order_by,'date_added',$order))?>">
            ...
            <tbody id="the-list">
                ...
                <tr id="post-1149" class="iedit author-self level-0 post-1149 type-page status-publish hentry simple-restrict-permission-wholesale">
                    ...
                    <td class="title column-title has-row-actions column-primary page-title" data-colname="Title">
                        ...
                        <strong>
                            <a class="row-title" href="<?=admin_url('admin.php?page=woocommerce-influencers&action=edit')?>"><?=$influencer->firstname.' '.$influencer->lastname?></a>
                        </strong>

                        
                        <div class="row-actions">
                            <span class="edit">
                                <a href="<?=admin_url('admin.php?page=woocommerce-influencers&id='.$influencer->id.'&action=edit')?>">Edit</a> | 
                            </span>
                            <?php if($influencer->balance>0): ?>
                            <span class="edit">
                                <a target="_blank" href="<?=admin_url('admin.php?page=woocommerce-influencers&id='.$influencer->id.'&action=payout&role='.$influencer->user_role)?>" class="warning">Payout</a> | 
                            </span> 
                            <?php endif; ?>
                            <span class="edit">
                                <a target="_blank" href="<?=$WI_Influencer->get_url('coupons','&as=admin&role=influencer&uid='.$influencer->id)?>" class="submitdelete danger" >Login as me</a>
                            </span>
                        </div>
                    </td>
                    <td class="column-cols">
