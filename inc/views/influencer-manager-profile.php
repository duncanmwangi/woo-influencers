...
if(isset($_POST['wi_save_changes'])){
	...
	if(empty($firstname) || empty($lastname) || empty($email) || empty($phone) || empty($paid_by) ||  ($paid_by=='paypal' && empty($paypal_email)) || ($paid_by=='bank_acount' && empty($dovetale_link)) || empty($currentpassword) ){
...
$user = wi_valid_user(wi_get_current_user_id(),'influencer-manager');
if($user):
...
<div class="row mx-0"> 
	<div class="col-md-8 mt-3 px-0">
		...
		<form action="" method="post">
		  ...
		  <div class="form-group row">
		    <label for="platform" class="col-sm-4 col-form-label">Head Shot</label>
		    <div class="col-sm-8">
		      <img src="<?=get_site_url().$user->headshot_path?>" class="wi_headshot_img" alt="">
		    </div>
		  </div>
